...
public class AppointmentServiceImpl implements AppointmentService {
    ...
    public List<Appointment> findAllByDentist(Dentist dentist) {
        return repository.findAll().stream()
                .filter(appointment -> appointment.getTreatments().stream()
                        .filter(treatment -> treatment.getDentist().getId().equals(dentist.getId())).toList().size()>0)
    ...
    public void removeTreatment(Long id, Treatment treatment) throws AppointmentNotFoundException {
        Appointment appointment = this.findById(id);
        if (!appointment.isFinalized()) {
            appointment.removeTreatment(treatment);
            this.repository.save(appointment);
        }
